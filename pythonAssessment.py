...
def count_specific_word(text, word):
    """Counts how many times a specific word appears in the text (case-insensitive)."""
...
def identify_most_common_word(text):
    """Returns the most frequently used word in the text using regex."""
    if not text:
        return None
...
def calculate_average_word_length(text):
    """Returns the average word length as a float, ignoring punctuation."""
    if not text:
        return 0
...
print(f"\n📌 Specific Word Count ('apple'): {count_specific_word(article, 'apple')}")
